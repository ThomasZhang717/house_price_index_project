...
gbm_hedonic_index_impute = data.frame(month = 1:6120, rate = 1, index = 100)
...
for (i in la_desc[,1]) {
  ...
  for (j in 1:203){
    ...
    temp_li = temp_hedonic[which(temp_hedonic$month_num == j),]
    ...
    gbm_hedonic_index_impute$index[(n-1)*204+j+1] = prod(gbm_hedonic_index_impute$rate[((n-1)*204+1):((n-1)*204+j+1)])*100
    print(j)
  }
  n=n+1
  print(i)
