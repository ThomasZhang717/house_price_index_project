...
#read data  #249,571 obs from 2015
```{r}
#read data
...
#picked property classes (*): 185,980
```{r}
#pick * classes
...
full_data$submarket[which(full_data$LA_DESC == "VINCENT                  " | full_data$LA_DESC == "SUBIACO                  " | full_data$LA_DESC == "NEDLANDS                 " | full_data$LA_DESC == "MOSMAN PARK              " | full_data$LA_DESC == "PEPPERMINT GROVE         " | full_data$LA_DESC == "CAMBRIDGE                " | full_data$LA_DESC == "CLAREMONT                " | full_data$LA_DESC == "COTTESLOE                ")] = "inner"
...
#check frequency of duplicates
#duplicates_fre = data.frame(table(duplicates$APP1))
#duplicates_delete_fre = data.frame(table(temp_duplicates$APP1))
#duplicates_fre$Freq1 = duplicates_delete_fre$Freq
#length(which(duplicates_fre$Freq == 3 & duplicates_fre$Freq1 == 1))
#length(which(duplicates_fre$Freq == 2 & duplicates_fre$Freq1 == 1))
#length(which(duplicates_fre$Freq == 4 & duplicates_fre$Freq1 == 2))
#length(which(duplicates_fre$Freq == 6 & duplicates_fre$Freq1 == 3))
#length(which(duplicates_fre$Freq == 32 & duplicates_fre$Freq1 == 16))
#compare the frequency
#pick group first
#2:3033 + 2*13 + 3*2 + 16 = 3081
#3:85
```
...
full_data_merge$quarter_num = (full_data_merge$year_sale - 2015)*4 + full_data_merge$quarter

#import index file
abs_index = read.csv(file = "./abs_index.csv")
...
#import spatial data
```{r}
spatial_data = read.dbf("./Transaction data/Cadastre_Polygon_Current.dbf")
```
...
full_data_merge$address = paste0(full_data_merge$ST_NO, full_data_merge$ST_NAME, full_data_merge$ST_SUFX, ",", full_data_merge$SUBURB, ", WA", sep = "")
...
#register_google(key = "AIzaSyADUkXIZR8qPPFdcHHVfvWHAUzw8N0YSUk")
#coords_na = which(is.na(full_data_merge$centlat))
#for (i in coords_na) {
...
```{r}
write.csv(full_data_merge, "./cleaned_data_2015_2020_paperversion_geocomplete_0tomiss.csv")
```



#read past geocoded data
```{r}
geocode_2015 = read.csv("./cleaned_data_2015_2020_paperversion_geocomplete.csv")[-1]
...
for (i in coords_na) {
  ...
  full_data_merge$centlat[i] = geocode_2015$centlat[which(geocode_2015$address == full_data_merge$address[i])[1]]
  full_data_merge$centlong[i] = geocode_2015$centlong[which(geocode_2015$address == full_data_merge$address[i])[1]]
  print(i)
}
#1 school st, kalamunda -31.9698041;116.0574984
```
